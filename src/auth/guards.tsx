...
export function RequireAuth() {
  ...
  if (!user) return <AuditRedirect reason="not_authenticated" attemptedPath={location.pathname} />
  return <Outlet />
}
...
export function RequireRole({ role, children }: { role: Role; children: React.ReactNode }) {
  ...
  const location = useLocation()
  if (!user) return <AuditRedirect reason="not_authenticated" attemptedPath={location.pathname} />
  if (user.role !== role) return <AuditRedirect reason="role_forbidden" attemptedPath={location.pathname} userId={user.uid} role={user.role} />
  return <>{children}</>
}
